        ...
        var JulbilantMetaModel = JSONModel.extend("jubilant.components.jubilantmetamodel.JubilantMetaModel", {
            ...
            _extractNavigationProperties: function(entityType) {
                ...
                oDataEntityType.navigationProperty.forEach(function(navigationProperty) {
                    ...
                    var associationEndType = associationEnd.type;
                    var associationEndTypeDescriptor = this.getEntityTypeDescriptor(associationEndType);
                    if (!associationEndTypeDescriptor) {
                        throw new Error("EntityType " + associationEndType + " for navigationProperty " + navigationProperty.name + " of type " + oDataEntityType.name + " not found.");
                    }
            ...
            _populateJubilantMetaModel: function() {
                ...
                metaModel.getODataEntityContainer().entitySet.forEach(function(entitySet) {
                    ...
                    var entityType = this.getEntityTypeDescriptor(typeName);
                    var entitySet = {
            ...
            _extractEntityTypeDescriptor: function(typeName) {
                var entityType = this.getEntityTypeDescriptor(typeName);
                var metaModel = this.getODataMetaModel();
                var oDataEntityType = metaModel.getODataEntityType(typeName);
                var baseType = oDataEntityType.baseType;
                if (baseType) {
                    baseType = entityType.baseType = this.getEntityTypeDescriptor(baseType);
                }
                entityType.oDataEntityType = oDataEntityType;
                var newArray;

                newArray = [];
                entityType.properties = baseType ? baseType.properties.concat(newArray) : newArray;

                newArray = [];
                entityType.navigationProperties = baseType ? baseType.navigationProperties.concat(newArray) : newArray;
            ...
            getEntityTypeDescriptor: function(entityTypeName) {
                ...
                if (!entityTypeDescriptor) {
                    ...
                    this.setProperty(path, entityTypeDescriptor);
                    this._extractEntityTypeDescriptor(entityTypeName);
                }
